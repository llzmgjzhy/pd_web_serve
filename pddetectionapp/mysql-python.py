...
def alter_table_add_timestamp_column(connection):
    with connection.cursor() as cursor:
        ...
        if not result:
            cursor.execute("ALTER TABLE sample_info ADD COLUMN data_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP")

        # 检查 sample_data 表中是否存在 data_time 列
        cursor.execute("SHOW COLUMNS FROM sample_data LIKE 'data_time'")
        result = cursor.fetchone()
        if not result:
            cursor.execute("ALTER TABLE sample_data ADD COLUMN data_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
...
def read_and_store_data(file_path, connection):
    with open(file_path, 'rb') as file:
        ...
        # # 连接到数据库并插入数据
        sample_info_id = insert_sample_info(connection, sensor_type, device_type, sampling_rate, sampling_length,
                           discharge_type)
...
def main():
    ...
    # Create tables
    create_table(connection, create_sampinfo_table)
    create_table(connection, create_sampdata_table)
    create_table(connection, create_pdresults_table)
